...
def main():
  ...
  if args.pre and args.pre[0] == "stop":
    commandline = f'(time {dlx}/dlx-pre < {args.template}.dlx) > {args.template}.pre.dlx '
  else:
    if args.pre and args.pre[0] == "pipe":
      commandpipe = f'{dlx}/dlx-pre < %s.dlx | {dlx}/%s m1 %s %s %s' % (args.template, executable, ticks, stop, verbose)
    else:
      commandpipe = (f'{dlx}/%s m1 %s %s %s < %s.dlx ' %
          (executable, ticks, stop, verbose, args.template))
    commandline = ('(time (%s)) 2>&1 | tee %s.out.txt | ggrep -v -P "^(\s(?!after)|\d+:|Level|L\d+)"' % (commandpipe, args.template))
